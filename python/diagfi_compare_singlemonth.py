...
for i in xrange(len(sigma)):
 alt[i] = -10*np.log(sigma[i])
alt_half = int(np.ceil(alt.shape[0]/1.5)) # 2/3's of the way up (half of actual altitude, but 2/3rds model level)

print "Latitude: %i || Longitude: %i || Model levels: %i => Alt Min:%.3f | Alt Max:%.3f | Alt two thirds (half actual altitude):%.3f ||| Level 14: %.3f || Level 10: %.3f" % (lat.shape[0],lon.shape[0],sigma.shape[0],alt[0],alt[-1],alt[alt_half],alt[15],alt[11])
...
c = np.matrix('4.01 45')#('0.5 45; 103.1 45; 242 2.5') # Dust storm mid-points [Ls Lat]

# Save destination

fpath = "/padata/alpha/users/aes442/results/Dustruns/m%i/" % (amth)
...
plt.savefig("%sheating.png" % (fpath))
plt.close('all')
...
alt_t = np.linspace(0,21,alt.shape[0]) # Height of 20.9km
