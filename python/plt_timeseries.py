...
def plt_timeseries(data, x, y, z, rows, cols, ticks, xlabel, ylabel, sp_titles, cb_label, zint, filename, d2_c=0):
...
# Ticks
    ...
    i_mjy = ticks[2]
    i_mny = ticks[3]
    major_ticksx = np.arange(np.min(x),np.max(x)+i_mjx, i_mjx)                                              
    minor_ticksx = np.arange(np.min(x),np.max(x)+i_mnx, i_mnx)  
    ...
    for i in xrange(rows):
     for j in xrange(cols):
      ...
      if (mola!=0):
       ax = axarr[i,j].contour(d2_c[1], d2_c[2], d2_c[2], lvls)
