...
wd = 12 # frequency of writediagfi per day 
...
for i in xrange(1,13,1):
 ...
 a = netcdf.netcdf_file("/home/physastro/aes442/padata/alpha/users/aes442/mars_gcms/MGCM_v5-1/runs/%s/%s/%s" % (rundira,month,filename),'r')
 b = netcdf.netcdf_file("/home/physastro/aes442/padata/alpha/users/aes442/mars_gcms/MGCM_v5-1/runs/%s/%s/%s" % (rundirb,month,filename),'r')
...
t = np.arange(1,669,1.0/wd) # Create new time dimension variable. Be careful to change interval if writediagfi interval is changed in runscript
...
temp_t = np.matrix.transpose(temp_diff[:,:,lon_slice])
